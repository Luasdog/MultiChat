/*
 ...
 Source Server         : multichatsql
 Source Server Type    : MySQL
 Source Server Version : 80300 (8.3.0)
 Source Host           : localhost:3308
 Source Schema         : multichat

 Target Server Type    : MySQL
 Target Server Version : 80300 (8.3.0)
 File Encoding         : 65001

 Date: 26/08/2025 15:00:01
*/
...
-- Records of user
-- ----------------------------
INSERT INTO `user` VALUES (1, 1001, 'test_admin', 'test_admin@example.com', 'test123');
...
BEGIN
    ...
    -- 检查用户名是否已存在
    IF EXISTS (SELECT 1 FROM `user` WHERE `name` = new_name COLLATE utf8mb4_general_ci) THEN
        SET result = 0; -- 用户名已存在
        COMMIT;
    ELSE
        -- 用户名不存在，检查email是否已存在
